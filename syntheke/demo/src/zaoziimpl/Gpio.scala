...
object GpioGen extends Generator[GpioP, GpioPLayers, GpioPIO, GpioPProbe]:
  def architecture(p: GpioP) =
    ...
    when(io.in.aw.valid & io.in.w.valid & (!bPending)) {
      ...
      when(wAddr === 0.B(2)) { outReg := wVal }
      when(wAddr === 1.B(2)) { dirReg := wVal }
    }
    ...
    when(io.in.ar.valid & (!rPending)) {
      ...
      rData := 0.B(32)
      when(rAddr === 0.B(2)) { rData := pad(outReg) }
      when(rAddr === 1.B(2)) { rData := pad(dirReg) }
      when(rAddr === 2.B(2)) { rData := pad(inSync) }
    }
