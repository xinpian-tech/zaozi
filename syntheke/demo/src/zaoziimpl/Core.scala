...
object CoreP:
  val xlen:         Int = 32
  val regIndexBits: Int = 4

class CorePLayers(p: CoreP) extends LayerInterface(p):
  def layers = Seq(Layer("Verification")) ++ Option.when(p.enableTrace)(Layer("DV"))
...
class CorePProbe(parameter: CoreP) extends DVBundle[CoreP, CorePLayers](parameter):
  ...
  val instructionTrace       = Option.when(parameter.enableTrace)(
    ProbeRead(new InstructionTrace(parameter.xlen, parameter.regIndexBits), dv)
  )
  val trace_clock             = Option.when(parameter.enableTrace)(ProbeRead(Clock(), dv))
  val trace_inReset           = Option.when(parameter.enableTrace)(ProbeRead(Bool(), dv))
  ...
  def architecture(p: CoreP) =
    ...
    if p.enableTrace then
      layer("DV"):
        ...
        sample.rdWe := rdWeW
        sample.rd := rdW
        sample.rdWdata := rdWdataW
        probe.instructionTrace.get <== sample
