...
/** The demo SoC's testbench device: terminates the UART's serial pins as a loopback jumper, reassembles the 8N1 frames
  ...
  * completed byte to [[SimConsole]], an external Verilog module whose behavioral definition [[simConsoleModel]] prints
...
val simConsoleModel: String = """`timescale 1ns / 1ps
module SimConsole (
    input       clock,
    input       valid,
    input [7:0] data
