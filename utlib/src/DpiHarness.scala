...
/** A harness that closes the DPI loop: each cycle it reads the DUT's Probe points, hands them to the external DPI
  * function through [[dpiCall]], and drives the DUT's input with the value the frontend returns. The probe reads and
  * the drive connection are ordinary DSL — the probe is just a wire in scope, so nothing here needs a hierarchical
  * reference.
  *
  * Single-drive DUTs only for now: the FIRRTL DPI call yields one result, so a DUT with more than one drive port needs
  * the multi-output shape, which is left for later.
  */
  ...
  def architecture(parameter: DpiHarnessParameter) =
    ...
    val observed    = spec.probe.map { port =>
      ...
      w <== instance
        ...
        .subfield[RProbe[Bits]](port.name)
      w
    }
