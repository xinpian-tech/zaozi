...
object Harvester:
  ...
  /** Reproduce a k-induction closure of an SVA implication-check by emitting the harvested
    * invariant in the checker's *sampled* frame.
    *
    * Yosys compiles `antecedent |-> consequent` into `concat(antecedent, consequent) == p`
    * sampled through a two-frame latch pipeline; its free latches let bare induction sustain a
    * phantom violation, so the strengthening must be stated one frame delayed, exactly as the
    * checker samples. This locates the antecedent (the concat-partner of the counter's `ult`),
    * the stage-2 latch (the state whose `next` feeds the `bad`), and the tail-state `eq` nodes,
    * then adds one-frame-delayed samples of `counter ≥ bound`, `fsm ∈ tail`, and the antecedent
    * with the three constraints that close it: C2 (high count ⟶ tail), C3 (tail ⟶ no
    * antecedent), C4 (stage-2 latch clear).
    */
  def closeSampled(design: Btor2Design, struct: Btor2Structure, tail: Set[Int]): Btor2Design =
    ...
    val inTail = tailEqs.reduceLeft { (acc, e) => fresh(); emit(s"$id or $sort1 $acc $e") }
