...
# %REPOROOT resolves to the zaozi project root so lit fixtures can reach
# repo-relative resources (e.g. circtlib/tests/resources/gcd.mlir) without
# embedding paths from out/. test_source_root is .../zaozi/lit/tests, so the
# project root is three directories up.
config.repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, os.pardir, os.pardir))
