...
/** Scoped destination for generator bytecode emitted during elaboration.
  *
  * The environment variable remains the command-line default; library users can use this scope to elaborate separate
  * designs in one JVM without mutating process-global state.
  */
object Elaboration:
  private val scopedOutputDirectory = new DynamicVariable[Option[os.Path]](None)

  def inOutputDirectory[T](directory: os.Path)(body: => T): T =
    scopedOutputDirectory.withValue(Some(directory))(body)

  private[zaozi] def outputDirectory: os.Path =
    scopedOutputDirectory.value.getOrElse(os.Path(sys.env.getOrElse("ZAOZI_OUTDIR", ""), os.pwd))
...
trait Generator[PARAM <: Parameter, L <: LayerInterface[PARAM], I <: HWInterface[PARAM], P <: DVInterface[PARAM, L]]:
  ...
  private[zaozi] val elaboratedModules = scala.collection.mutable.HashSet.empty[(PARAM, os.Path)]
...
final case class VerilogSource(fileName: String, content: String):
  require(fileName.nonEmpty, "Verilog source filename must not be empty")
  require(content.nonEmpty, "Verilog source content must not be empty")
...
trait VerilogWrapper[
  ...
  /** Inline blackbox sources lowered by CIRCT to `sv.verbatim.source` operations. */
  def verilogSources(parameter: PARAM): Seq[VerilogSource] = Seq.empty
