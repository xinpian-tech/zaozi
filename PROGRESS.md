...
| 提交 | 日期 | 内容 |
|------|------|------|
| vptqoszq | 2026-03-18 | [done] 过滤掉带 ISA 后缀的指令变体（如 bclri.rv32），减少 13 条重复指令 |
| ztvmsnpw | 2026-03-18 | [done] 修复 getInstructions() 排序与 getMergedInstructions() 不一致导致测试失败，RecipeTest 改为语义断言 |
| nwzmzwzv | 2026-03-18 | [done] 合并 RVConstraints 为单文件，添加 asm2dsl.py 脚本，更新 UpdateRVConstraints 生成单文件 |
...
## [done] Implement rvprobe round-trip test framework: add RISC-V cross-toolchain to flake.nix, create roundtrip test scripts (baremetal.ld, wrap_dsl.py, compare_objdump.py, roundtrip-test.sh), and fix asm2dsl.py section directive parsing
- Started: 2026-03-18 13:30 UTC
- Completed: 2026-03-18 14:27 UTC
- Summary: Round-trip test passes (113 instructions match); fixed asm2dsl for CSR/FP/rounding-mode/store-operand-order; fixed RVGenerator rendering for j, beqz, shamtd, CSR names, FP registers, and rounding modes; only la/li remain as pseudo

## [done] Z3 求解结果随机化
- Completed: 2026-03-19
- Summary: 在 `RVGenerator.toZ3Output()` 中注入 `smt.random_seed` 和 `sat.random_seed` 选项，使每次求解产生不同结果。添加 `val seed: Int` 字段（默认随机，子类可 override 固定）。注意 Z3 要求 seed 为无符号整数，`phase_selection 5` 会导致大规模公式返回 unknown 故未启用。
