...
/** A white-box internal status signal (e.g. a Queue's empty/full/almostFull),
  * exposed as a first-class constraint dimension. */
final case class StatusSignal(name: String, category: String)

/** A structured description of a Zaozi DUT's transaction interface.
  *
  * Today this is supplied directly (or hand-derived from a module's `QueueIO`
  * style typed IO). Auto-deriving it from the Chisel/Zaozi Object Model is
  * idea#9 L1; because Zaozi lowers to the same CIRCT IR as the constraints, the
  * ports and status signals are first-class there. */
...
/** The Zaozi leg's solved artifact: the frontend-agnostic [[SolvedSequence]]
  * plus the concrete transaction list the ChiselSim backend renders. */
final case class ZaoziArtifact(
  sequence:     SolvedSequence,
  transactions: Seq[Transaction]
) extends SolvedArtifact

/** The Zaozi leg of the [[DutFrontend]] contract — a transaction-level frontend
  * for Decoupled Zaozi modules (Queue, FIFO, arbiters, …). It is the second leg,
  * and it proves the contract generalizes beyond RISC-V:
  *
  *   - `alphabet` — transaction kinds derived from the Decoupled ports
  *     (Drive ⇒ enqueue, Monitor ⇒ dequeue), not an instruction set.
  *   - `whitebox` — the module's internal status signals (empty/full/…), the
  *     Decoupled-world analogue of RISC-V µarch predicates.
  *   - `backend` — a ChiselSim-style poke/peek/step driver, not GAS assembly.
  *
  * SCOPE (honest): `solve()` currently emits a straightforward deterministic
  * smoke sequence (drive every Drive port once, drain every Monitor port once).
  * The SMT-backed *sequence-level* solving of transactions under Decoupled
  * handshake timing — the "adjacency" semantics of valid/ready, backpressure,
  * and coverRAW/WAR/WAW lifted from registers to addresses — is idea#9's L2
  * research core. When built, it replaces the body of `solve()` and reuses this
...
final class ZaoziFrontend(iface: TransactionInterface) extends DutFrontend:
  ...
  /** PLACEHOLDER sequence (see class scope note): drive each Drive port once
    * with a small distinct value, then drain each Monitor port once. The
    * SolvedSequence records the transaction selection/fields so downstream
    * tooling sees the same shape it will once the SMT solver is wired. */
