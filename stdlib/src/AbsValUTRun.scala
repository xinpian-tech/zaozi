...
/** Command-line runner the `AbsValUT.sc` lit test drives: harness the [[AbsValUT]] module and
  * emit the UT testbench, DPI contract, shim, and closed-loop harness.
  *
  * This is a stop-gap entry — the run flow should eventually be a Generator capability rather
  * than a hand-written main — so it is deliberately thin and unpolished.
  */
object AbsValUTRun:
  def main(args: Array[String]): Unit =
    ...
    val parameter = AbsValUTParameter(width)
    ...
    os.write.over(
      ...
      s"""|#include <stdio.h>
          |extern "C" {
          |static int cyc = 0;
          |void ${AbsValUT.moduleName(parameter)}_tick(char a, char absval, char* A) {
    ...
    println(s"UT testbench, DPI contract, shim and closed-loop harness emitted for ${AbsValUT.moduleName(parameter)} in $outDir")
