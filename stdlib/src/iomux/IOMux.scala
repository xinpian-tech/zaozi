...
case class IOMuxParameter(
  ...
  require(impId >= 0 && impId.bitLength <= 64, "impId must fit an unsigned 64-bit word")
  require(dataWidth >= 8 && (dataWidth & (dataWidth - 1)) == 0, "dataWidth must be a power of two and at least 8")
  require(
  ...
  private def bankSizes(names: Seq[String], count: Int): Seq[(String, BigInt, BigInt)] =
    names.map(name => (name, arrayBytes(count, 1), BigInt(0)))

  private val blockSizes   =
    Seq(("hs_select", arrayBytes(pinCount, selectorLaneWidth), BigInt(0))) ++
      (if option.gpio then bankSizes("input_value" +: roles, pinCount) else Seq.empty) ++
      (if option.rxOverride then bankSizes((0 until hsSlots).map(s => s"rx_value_s$s"), pinCount) else Seq.empty) ++
      (if option.interrupt then
         bankSizes(irqKinds.map(k => s"${k}_int_en") ++ irqKinds.map(k => s"${k}_int_pend"), pinCount)
       else Seq.empty) ++
      ...
      Seq(
        ("ls_select", if lsPools.nonEmpty then arrayBytes(pinCount, lsSelectorLaneWidth) else BigInt(0), BigInt(0)),
        ("ls_rx_pin", arrayBytes(lsChannelCount, lsRxLaneWidth), BigInt(0))
      ) ++
      (if option.rxOverride && lsPools.nonEmpty then bankSizes(Seq("ls_rx_src", "ls_rx_value"), lsChannelCount)
       else Seq.empty) ++
  ...
  val blocks               = blockSizes
    .zip(blockOffsets)
    .map:
      case ((name, bytes, stride), base) => (name, base, bytes, stride)
  val windowBytes          = blockOffsets.last
  private val blockBases   = blocks.map((name, base, _, _) => name -> base).toMap
  ...
  private def bank[F <: RegFieldDefinition](name: String, indices: Seq[Int])(field: Int => F): Map[Int, F] =
    val base = blockBases(name)
    indices.map(i => i -> place(base * 8 + i, field(i))).toMap

  val identity = (0 until 8).map: byte =>
    (BigInt(byte), place(BigInt(byte) * 8, RegField(s"impid_$byte", 8).readValue), (impId >> (byte * 8)) & 255)
  ...
  val selectorBase = blockBases("hs_select")
  val selectors    =
    pins.map(p => selectorFields(s"pin_${p}_select", selectorWidth, selectorBase * 8 + BigInt(p) * selectorLaneWidth))

  val gpioInputValue =
    if option.gpio then bank("input_value", pins)(p => RegField(s"pin_${p}_input_value", 1).readValue)
    else Map.empty[Int, ReadFieldDefinition[ValueReadDefinition]]
  val gpio           = roles
    .flatMap(name =>
      Option.when(option.gpio)(
        name -> bank(name, pins)(p => RegField(s"pin_${p}_$name", 1).readValue.writeValue)
      )
    )
    .toMap
  val rxValues       =
    if option.rxOverride then
      (0 until hsSlots).map(slot =>
        bank(s"rx_value_s$slot", pins)(p => RegField(s"pin_${p}_rx_value_s$slot", 1).readValue.writeValue)
      )
    else Seq.empty
  val irqEnables     = irqKinds
    .flatMap(kind =>
      Option.when(option.interrupt)(
        kind -> bank(s"${kind}_int_en", pins)(p => RegField(s"pin_${p}_${kind}_int_en", 1).readValue.writeValue)
      )
    )
    .toMap
  val irqPending     = irqKinds
    .flatMap(kind =>
      Option.when(option.interrupt)(
        kind -> bank(s"${kind}_int_pend", pins)(p =>
  ...
  val roleInverts    = roles
    .flatMap(name =>
      Option.when(option.invert)(
        name -> bank(s"${name}_inv", pins)(p => RegField(s"pin_${p}_${name}_inv", 1).readValue.writeValue)
      )
    )
    .toMap
  val rxInverts      =
    if option.invert then
      (0 until hsSlots).map(slot =>
        bank(s"rx_inv_s$slot", pins)(p => RegField(s"pin_${p}_rx_inv_s$slot", 1).readValue.writeValue)
      )
    else Seq.empty
  ...
  val lsSelectorBase = blockBases("ls_select")
  ...
  val lsRxBase       = blockBases("ls_rx_pin")
  val lsRxPins       =
  ...
  val lsRxSources    =
    if option.rxOverride && lsPools.nonEmpty then
      bank("ls_rx_src", receivers)(c => RegField(s"ls_c${c}_rx_src", 1).readValue.writeValue)
    else Map.empty[Int, ReadWriteFieldDefinition[ValueReadDefinition, ValueWriteDefinition]]
  val lsRxValues     =
    if option.rxOverride && lsPools.nonEmpty then
      bank("ls_rx_value", receivers)(c => RegField(s"ls_c${c}_rx_value", 1).readValue.writeValue)
    else Map.empty[Int, ReadWriteFieldDefinition[ValueReadDefinition, ValueWriteDefinition]]
  val lsRxInverts    =
    if option.invert && lsPools.nonEmpty then
      bank("ls_rx_inv", receivers)(c => RegField(s"ls_c${c}_rx_inv", 1).readValue.writeValue)
    else Map.empty[Int, ReadWriteFieldDefinition[ValueReadDefinition, ValueWriteDefinition]]

  val sourceBase     = blockBases("pin_src_ctrl")
  ...
  require((windowBytes - 1).bitLength <= addressWidth, "IOMux register window does not fit addressWidth")

  private val occupiedBytes = fieldLayout.flatMap: (bit, field) =>
    bit / 8 to (bit + field.width - 1) / 8
  private val padding       = (BigInt(0) until windowBytes)
    .filterNot(occupiedBytes.toSet)
    .map: byte =>
      (byte * 8, RegField.reserved(s"reserved_byte_$byte", 8))
  private val words         = (fieldLayout ++ padding)
    ...
    .map: (word, entries) =>
      val definitions = entries
        .sortBy(_._1)
        .foldLeft((word * dataWidth, Vector.empty[RegFieldDefinition])):
          case ((end, definitions), (bit, field)) =>
            ...
            (bit + field.width, definitions ++ gap.toSeq :+ field)
      RegMapRegister(word * (dataWidth / 8), definitions._2)
...
object IOMux extends Generator[IOMuxParameter, IOMuxLayers, IOMuxIO, IOMuxProbe]:
  ...
  def main(args: Array[String]): Unit = args.toList match
    case "header" :: config :: Nil =>
      ...
      (Seq("IMPID_OFFSET" -> BigInt(0), "IMPID_VALUE" -> p.impId, "IMPID_WIDTH" -> BigInt(64)) ++ arrays)
        .foreach: (name, value) =>
  ...
  def architecture(parameter: IOMuxParameter) =
    ...
    val rsp             = Wire(Decoupled(new RegMapResponse(parameter.dataWidth, true)))
    val wordIndex       = io.req.bits.address.bits(parameter.addressWidth - 1, parameter.byteBits).asUInt
    val aligned         = if parameter.byteBits == 0 then true.B else !io.req.bits.address.bits(parameter.byteBits - 1, 0).orR
    val inWindow        = io.req.bits.address.asUInt <= (parameter.windowBytes - 1).U(parameter.addressWidth)
    val responseZero    = Reg(Bool())
    val responseOutside = Reg(Bool())
    responseZero          := io.req.fire ? (!(aligned & inWindow), responseZero)
    responseOutside       := io.req.fire ? (!inWindow, responseOutside)
    req.bits.index        := wordIndex
    req.bits.read         := io.req.bits.read
    req.bits.data         := io.req.bits.data
    req.bits.mask         := (aligned & inWindow) ? (
      io.req.bits.read ? (((BigInt(1) << (parameter.dataWidth / 8)) - 1).B(parameter.dataWidth / 8), io.req.bits.mask),
      BigInt(0).B(parameter.dataWidth / 8)
    )
    req.valid             := io.req.valid
    io.req.ready          := req.ready
    rsp.ready             := io.rsp.ready
    io.rsp.valid          := rsp.valid
    io.rsp.bits.read      := rsp.bits.read
    io.rsp.bits.data      := responseZero ? (BigInt(0).B(parameter.dataWidth), rsp.bits.data)
    io.rsp.bits.error.get := responseOutside | rsp.bits.error.get
    ...
    val accesses = parameter.identity.map((_, field, value) => field.read(value.B(field.width))) ++
      hsRegisters.flatMap(_._2) ++ (lsRegisters ++ lsRxRegisters).flatMap(_._2._2) ++ extraAccesses
    parameter.regMap(req, rsp)(accesses*)
