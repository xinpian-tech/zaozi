...
// RUN: %{test} config %t.json --stages 2 --polarity active-high
// RUN: %{test} design %t.json
// RUN: firld *.mlirbc --base-circuit SynchronizedReset_stages2_activeHigh --no-mangle | firtool --format=mlir | FileCheck %s
