...
#table(
  ...
  [结构模块], [`WrapperModule`], [只含子模块、节点与连接的模块；电路体由框架发射（@sec-module-kinds、@sec-wrapper-emission）。],
...
#table(
  ...
  [@req-interconnect], [逻辑总线拓扑独立描述、始终保留；物理互连（xbar/NoC/直连）按区域承载、可替换（机制设计进行中）], [设计讨论中],
)
...
#table(
  ...
  [@dec-dv-once], [探针连接只允许单连。], [@sec-dv-routing],
...
这份文档从"接口参数是拓扑的全局函数"这一个观察出发，推出了两张图、三个阶段、一条序列化边界；协议给出参数的语义，角色与算子给出图的形状，协商把二者算成一份可序列化、可复现、可审计的结果，例化只是照着这份结果生成电路。每一处形式化——共享解规则、端口区间、结构键、层的前缀树并——都服务于同一个目标：*使每一个字节的来源都可以被追溯*。实现应当以本文档为契约；文档未及之处，以#ref(<sec-requirements>)的七条需求为裁决依据。
