...
#table(
  ...
  [连接结构], [connection structure], [总线、agent 与 bind 构成的结构，独立于层次（@sec-two-graphs、@ch-interconnect）。],
  [总线], [bus], [命名一组 agent 的逻辑分组；承载可达、地址、域，并在其上协商，不产硬件（@sec-bus-fabric）。],
  [fabric], [—], [承载一条总线的硬件生成器（crossbar／NoC／直连）；一条总线恰好对应一个（@sec-bus-fabric）。],
  [agent], [—], [接到总线上的一个 IP：发起者、响应者，或连接两条总线的桥；对总线而言都是同一种东西（@sec-attach）。],
  [桥], [bridge], [连接两条平总线的设备：一侧为响应者、一侧为发起者，内部做地址转换/跨时钟域/位宽转换；有意义的边界（独立交付、域改变、换 fabric）即一个桥（@sec-flat-nest）。],
  [节点], [node], [模块在某个协议上的一个端口，带协议与方向；一次 bind 把它接到总线的一个具名节点上，它求解出的接口就是它的 IO 线形状（@sec-attach、@sec-generator-module）。],
  [边], [edge], [协商后一条点对点链路；每次 bind 求解出一条边（@sec-settle-pp）。],
  [构建器令牌], [builder token], [框架入口注入的上下文值；bind 在其作用域外无法编译（@sec-triptych）。],
  [协议], [protocol], [定义 `Down`/`Up`/`Edge` 三类型与求解函数的对象（@sec-protocol-object）。],
  [跨协议 adaptor], [cross-protocol adaptor], [两侧服从不同协议、携带跨协议变换的 adaptor，置于桥内（@sec-flat-nest；机制见 @sec-protocol-object）。],
  ...
  [结构模块], [`WrapperModule`], [只含子模块、节点与 bind 的模块；电路体由框架发射（@sec-module-kinds、@sec-wrapper-emission）。],
  ...
  [角色], [role], [agent 的方向：发起者或响应者；聚合由总线承担，跨协议或跨域的转换落在桥内（@sec-flat-nest）。],
  [bind], [—], [`bus.node("名") <- 设备`：唯一的连接原语，把设备的一个节点 bind 到总线落点上；bind 上一个节点就接上了它的端口/IO，数量即 bind 的个数（@sec-attach）。],
  [具名节点], [named node], [fabric 暴露的物理落点，一个端口或路由器，以名字标识（@sec-placement）。],
  [落点], [placement], [agent 落在哪个具名节点；构建期显式给出，协商不读，例化期消费（@sec-placement）。],
  [清单], [manifest], [总线交给 fabric 的数据：agent 表与可达表；唯一的一次交接（@sec-manifest）。],
...
#table(
  ...
  [@req-iteration], [基于 eDSL 的设计源码是唯一实施来源；架构图从源码导出；三阶段把静态可查错误全部前置到协商期；数量即 bind 的个数，增删设备只增删 bind、不改任何数量声明], [@sec-triptych、@sec-attach、@sec-visualization、@sec-error-semantics],
  [@req-negotiation], [两端指认与双向参数流；逐边求解；错误在协商期报出], [@sec-three-param-kinds–@sec-protocol-object、@sec-propagation],
  ...
  [@req-interconnect], [逻辑结构在总线上描述一次、始终保留；fabric（crossbar／NoC／直连）按总线承载、可替换；落点显式给出], [@ch-interconnect],
)
...
这份文档从"接口参数是拓扑的全局函数"这一个观察出发，推出了两张图、三个阶段、两种模块、一条序列化边界；协议给出参数的语义，总线与 bind 给出连接结构的形状，协商把二者算成一份可序列化、可复现、可审计的结果，例化只是照着这份结果生成电路。每一处形式化——地址图、清单、结构键、层的前缀树并——都服务于同一个目标：*使每一个字节的来源都可以被追溯*。实现应当以本文档为契约；文档未及之处，以#ref(<sec-requirements>)的七条需求为裁决依据。
