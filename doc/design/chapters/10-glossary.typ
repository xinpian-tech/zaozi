...
#table(
  ...
  [构建上下文], [`DesignBuilder`], [`design` 入口注入的构建期上下文；节点声明与 bind 通过它写入 `DesignSpec`（@sec-triptych）。],
  ...
  [`dFn` 与 `uFn`], [port parameter functions], [输出节点从显式前驱输入节点的 `Down` 计算本节点 `Down`，输入节点从显式后继输出节点的 `Up` 计算本节点 `Up` 的确定性函数（@sec-node-conn-proto、@sec-propagation）。],
  ...
  [探针源与探针汇], [DV source and DV sink], [生成器模块声明的验证端点；源提供 `Down`，汇按 bind 声明顺序收集并执行一次 `resolve`（@sec-dv-declarations、@sec-dv-model）。],
...
#table(
  ...
  [@req-iteration], [eDSL 源码定义连接和参数；架构图由源码导出；静态连接错误在协商期报告], [@sec-triptych、@sec-attach、@sec-visualization、@sec-error-semantics],
