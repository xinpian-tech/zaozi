...
#table(
  ...
  [fabric], [—], [兑现一条总线的硬件生成器（crossbar／NoC／直连）；一条总线恰好对应一个（@sec-bus-fabric）。],
  ...
  [角色], [role], [成员的方向：发起者或响应者；聚合由总线承担，跨协议或跨域的转换落在桥内（@sec-flat-nest）。],
  ...
  [清单], [manifest], [总线交给 fabric 的数据：成员表与可达表；唯一的一次交接（@sec-manifest）。],
...
#table(
  ...
  [@req-iteration], [基于 eDSL 的设计源码是唯一实施来源；架构图从源码导出；三阶段把静态可查错误全部前置到协商期；数量即附着的个数，增删设备只增删附着、不改任何数量声明], [@sec-triptych、@sec-attach、@sec-visualization、@sec-error-semantics],
...
这份文档从"接口参数是拓扑的全局函数"这一个观察出发，推出了两张图、三个阶段、两种模块、一条序列化边界；协议给出参数的语义，总线与附着给出连接结构的形状，协商把二者算成一份可序列化、可复现、可审计的结果，例化只是照着这份结果生成电路。每一处形式化——地址图、清单、结构键、层的前缀树并——都服务于同一个目标：*使每一个字节的来源都可以被追溯*。实现应当以本文档为契约；文档未及之处，以#ref(<sec-requirements>)的七条需求为裁决依据。
