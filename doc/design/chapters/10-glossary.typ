...
#table(
  ...
  [IO 节点], [IO node], [长在设计边界上的普通节点，只在顶层声明；经普通 bind 与内部节点协商出顶层 IO 的形状（@sec-io-nodes）。],
  [探针清单], [probe manifest], [`DesignSpec` 的纯函数：全部探针源按模块先序与声明顺序、每叶一条的可序列化记录（@sec-dv-testbench）。],
  [测试平台], [testbench], [例化期可选例化进根模块的自由硬件：无参数，读查询接口；主体实例驱动顶层 IO，每层一个探针实例（@sec-dv-testbench）。],
