...
#决策([探针自动上提到根，图内不设收集端])[
  探针源声明即完成接线：框架把每个探针叶从源模块逐层上提，直至设计根。收集端不是图的节点——若是，它就要参与协商、要求解聚合接口，而它的形状本是设计的确定函数。消费发生在图之外：例化期的测试平台在根内读取探针（@sec-dv-testbench），或由顶层探针端口交给设计外的工具。
] <dec-dv-top>
...
#term[测试平台][testbench]在例化期可选地例化进根模块，消费设计的全部边界：顶层 IO（@sec-io-nodes）由它驱动和接收，探针由它读取。它不在图里、不参与协商、没有参数——它的形状是设计的确定函数，序列化没有意义；作者持有协商结果，用查询接口读元数据，自由地写硬件：

- `ios`：每个 IO 节点的名字、方向、协议与已求解边（含接口和参数的类型化读取）；
- `probes`：#term[探针清单][probe manifest]——每个探针源的标识、`Down`、层路径，以及每叶的端口名（@sec-port-naming 的 Dangle 名）与 `ref.resolve` 后的数据类型。清单是 `DesignSpec` 的纯函数，可序列化，供工具共用。

FIRRTL 决定了测试平台的形状：从探针 resolve 出的值只能在对应层的 layerblock 里使用，出不了块；反过来 layerblock 里的逻辑不得驱动外部信号。所以驱动 IO 的部分与读探针的部分分属不同实例——主体实例常在，每个 IO 节点对应一个同接口、根方向相反的端口，按 IO 名命名；每个层一个探针实例，置于该层 layerblock 内，每叶一个数据输入，按清单端口名命名，其模块因此不得再含 layerblock。两部分的关联在 DPI 等软件侧汇合。框架负责例化位置、逐叶 `ref.resolve`、按名连线，并以与生成器相同的绑定检查点当场核对端口（@dec-binding-check）。

不例化测试平台时，IO 成为根模块的顶层端口，探针路径终止于顶层探针端口，供设计外的消费者按 FIRRTL 引用 ABI 接入。
