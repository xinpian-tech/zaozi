...
#term[测试平台后端][testbench backend]就是普通的生成器后端：harness 的完整参数是按层切分的清单（层路径加该层的探针源），模块按链接键命名、由链接器并入设计 circuit——接口从参数导出，与任何生成器一致。对每个层，框架在根模块内打开对应 layerblock 例化 harness；契约是每叶一个输入端口，以清单端口名命名、类型为该叶的数据类型，除此之外没有端口。框架逐叶 `ref.resolve` 后按名连入，与生成器子实例共用同一个绑定检查点（@dec-binding-check）。harness 不在图里、不参与协商，形状是设计的确定函数：清单从规格流向它，方向单一。
