...
构建期查询接口返回此前声明的全部探针源——每源的标识、`Down` 与层路径，每叶的端口名（@sec-port-naming 的 Dangle 名）与数据类型（#term[探针清单][probe manifest]，`DesignSpec` 的纯函数，可序列化）。测试平台作者用它决定探针端口和完整参数；协商结果侧的同一份数据供设计外工具使用。
