...
`computeFullParam` 将框架提供的 `EdgeView` 与闭包中的用户参数直接合成 `FullParam`。独立例化按名字找到注册项，并用该项的序列化解码完整参数。`FullParam` 必须足以确定生成器全部设计端口与探针端口的名称、方向和接口结构。对于构建期动态声明的节点，`computeFullParam` 把节点名称、方向和 `ProtocolBundle` 约束转换为生成器自有字段，在 `FullParam` 中保留复现这些接口所需的值；若节点名称由生成器的固定接口决定，`FullParam` 只需保存决定接口结构的参数。
...
#决策([端口结构校验在例化期进行])[
  生成器的设计端口和验证端口必须与相应 `ProtocolBundle` 完全一致：设计 bind 的源端根方向为 Output，目标端为 Input；探针源按接口叶展开，每叶一个 Output 纯 `Probe` 端口，名称为源名加叶路径段（@sec-port-naming）；字段名称、顺序和方向（`Flipped`），`Bundle`、`Vec`、`UInt`、`SInt`、`Bool`、`Clock`、`Reset`、`Probe` 类型构造器，Vec 长度、整数宽度与符号，以及 Probe 的 `LayerPath` 均逐层相同。声明端口缺失、参与连线的顶层 Bundle 没有对应声明或结构失配时，错误包含端点稳定标识、声明的源码位置以及期望结构与实际结构的差异路径。
] <dec-binding-check>
...
生成器模块的定义与其实例引用按模块名链接，模块名因此是链接键：它必须忠实编码（生成器名字、规范化完整参数），不同身份不得产生相同模块名。链接同时把各 per-module circuit 中的 FIRRTL 层定义并入设计 circuit——生成器可以携带与探针路由无关的内部层。

生成器内部还可以实例化真外部 Verilog 模块：它以 extmodule 声明进入电路，声明即定义，链接原样保留。RTL 无法企及之处——时钟的起源、仿真的输出、调试器的线缆——由这类模块的行为级定义补上，与发射出的 Verilog 一起交给仿真器。仿真之外的工具由此可以接进来：线缆的另一端是真的调试器，设计里不必再放一个。
