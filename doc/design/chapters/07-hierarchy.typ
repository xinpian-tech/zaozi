...
设连接 $c$ 的源端位于生成器模块 $A$，目标端位于生成器模块 $B$。当 $A != B$ 时，连线作用域 $W$ 取 $A$ 与 $B$ 的最近公共祖先（lowest common ancestor，`LCA(A, B)`）；当 $A = B$ 时，$W$ 取 $A$ 的父模块。结构模块 $W$ 发射两端之间的连接。设计连接的源端与目标端分别来自 bind 的源、目标节点。一端是顶层 IO 节点时（@sec-io-nodes），该端没有分支：它就是根模块自己的端口，名字取 IO 节点名，方向与该侧分支方向相反——outward IO 读顶层 Input，inward IO 驱动顶层 Output。验证路径不设 $W$，端口一路生成到设计根（含根）。规划规则：
...
根结构模块的端口是顶层 IO（@sec-io-nodes）与顶层探针端口，且仅在不例化测试平台时存在——例化时二者都被测试平台终结，Top 完全封闭（@sec-dv-testbench）。根模块是仿真顶层的容器：芯片顶层是它的子模块。DDR、PCIe 等外部模型可以作为根下的生成器模块进图，也可以经 IO 节点交给测试平台驱动（@sec-io-nodes）。
