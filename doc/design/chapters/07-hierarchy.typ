...
每条已求解连接具有两个生成器模块端点。对于设计 bind，两端是源输出节点与目标输入节点所标识的生成器端口（@sec-interconnect-nodes）；对于验证 bind，两端分别是探针源生成器与探针汇生成器（@sec-dv-declarations）。层次路径由这两个硬件端点确定。
...
+ 连接的两个端点是两端生成器的端口。设计连接使用两端的完整顶层 Bundle；验证连接使用源端的完整顶层 Bundle 和汇端选中的 Bundle。为了让连接穿过中间的结构模块，框架在这些模块上生成端口，称为#term[转发端口][forwarding port]：从 $A$ 的父模块开始逐层上行，在到达 $W$ 之前为每个模块边界生成一个 Output 转发端口；从 $B$ 的父模块开始逐层上行，在到达 $W$ 之前为每个模块边界生成一个 Input 转发端口。
...
- *转发端口基段。*设计边源端对应 `["node", 节点声明名, "out"]`，目标端对应 `["node", 节点声明名, "in"]`；探针源对应 `["dv-source", 源名, "out"]`，探针汇对应 `["dv-sink", 汇名, "in"]`（@sec-attach、@sec-dv-declarations）。每个模块节点恰好对应一条边，节点声明名和方向足以确定基段。这些基段用于框架生成的转发端口，不改变生成器端点的原始名称。
...
例如：生成器模块 `l2` 内输出节点 `mem` 的实际端口名为 `mem`。`l2` 在结构模块 `soc` 中的实例名也为 `l2`；当该连接需要穿过 `soc` 边界时，`soc` 的转发端口名称段为 `["inst", "l2", "node", "mem", "out"]`。
...
- 结构模块的键 $=$（端口序列，子实例序列，连线序列，FIRRTL 层声明）的规范化序列化。端口按编码名称排序，条目包含根方向与完整 `ProtocolBundle`，Bundle 字段保留协议定义顺序；子实例全部来自设计源码中的显式子模块并保持声明顺序，条目包含实例名与所引用模块的结构键；每条连线以本结构模块内的局部端点引用表示端点，即本层端口名或“子实例名 + 子端口名”，并保留两端方向；连线列表按源、目标局部端点引用组成的二元组排序。全局 `ModuleId`、`ModuleNodeId`、`BindId` 与声明位置只用于追踪和诊断，不进入结构键。FIRRTL 层按路径排序。例化阶段使用相同顺序，结构模块的键按层次树自底向上计算。
