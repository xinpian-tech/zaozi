...
+ 连接的两个端点是两端生成器的端口。设计连接使用两端的完整顶层 Bundle；验证连接使用源端的完整顶层 Bundle 和汇端选中的 Bundle。为了让连接穿过中间的结构模块，框架在这些模块上生成端口，称为#term[Dangle 端口][dangle]：从 $A$ 的父模块开始逐层上行，在到达 $W$ 之前为每个模块边界生成一个 Output 方向的 Dangle 端口；从 $B$ 的父模块开始逐层上行，在到达 $W$ 之前为每个模块边界生成一个 Input 方向的 Dangle 端口。
...
- *Dangle 端口基段。*设计边源端对应 `["node", 节点声明名, "out"]`，目标端对应 `["node", 节点声明名, "in"]`；探针源对应 `["dv-source", 源名, "out"]`，探针汇对应 `["dv-sink", 汇名, "in"]`（@sec-attach、@sec-dv-declarations）。每个模块节点恰好对应一条边，节点声明名和方向足以确定基段。这些基段用于框架生成的 Dangle 端口，不改变生成器端点的原始名称。
