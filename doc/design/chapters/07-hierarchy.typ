...
设连接 $c$ 的源端位于生成器模块 $A$，目标端位于生成器模块 $B$。当 $A != B$ 时，连线作用域 $W$ 取 $A$ 与 $B$ 的最近公共祖先（lowest common ancestor，`LCA(A, B)`）；当 $A = B$ 时，$W$ 取 $A$ 的父模块。结构模块 $W$ 发射两端之间的连接。设计连接的源端与目标端分别来自 bind 的源、目标节点。验证路径不设 $W$——不声明测试平台时端口一路生成到设计根（含根），声明时在根内接进测试平台实例（@sec-dv-testbench）。规划规则：

+ 连接的两个端点是两端生成器的端口。设计连接使用两端的完整顶层 Bundle；验证连接按源接口的接口叶展开，每叶一条纯 `Probe` 路径（@sec-dv-routing）。为了让连接穿过中间的结构模块，框架在这些模块上生成端口，称为#term[Dangle 端口][dangle]：从 $A$ 的父模块开始逐层上行，在到达 $W$ 之前为每个模块边界生成一个 Output 方向的 Dangle 端口；从 $B$ 的父模块开始逐层上行，在到达 $W$ 之前为每个模块边界生成一个 Input 方向的 Dangle 端口。验证路径没有目标分支：Output Dangle 端口逐层生成，直到设计根（含根），根上的 Dangle 端口即顶层探针端口。
...
根结构模块没有自己的端口，设计是封闭电路；唯一例外是不声明测试平台时的顶层探针端口（@sec-dv-testbench）。根模块是仿真顶层的容器：芯片顶层是它的子模块，DDR、PCIe 等外部模型与测试平台都是根下的生成器模块。
...
声明名称（实例名、节点名与探针源名、层段名）原样成为 FIRRTL 符号，形状限定为 `[A-Za-z_][A-Za-z0-9_]*`，在声明处检查。用户名称不含 `-` 与 `$`，因此可逆编码的转义字符与 `dv-source` 等框架段名和用户名称空间构造性隔离。
...
结构模块每实例发射一份模块定义，模块名在声明处显式给出，根为 `Top`。结构相同模块的网表级合并交给 FIRRTL 编译流程自带的去重，Syntheke 不维护结构键。

#决策([结构模块的模块名显式声明])[
  生成器模块的模块名由链接键决定，那是模块本身的身份；结构模块没有完整参数可供取名，它的内容就是模块体组合出的东西，只有作者知道该叫什么。模块名因此是 `wrapper` 的显式参数，实例名仍取自绑定它的 val。同一设计中两个结构模块不得取同一模块名，根的 `Top` 同样参与该检查（@sec-structural-check）；结构模块名也不得与本设计某个生成器模块发射的模块名相同，该检查在例化期生成器模块名确定后进行（@sec-elaboration-flow）。
