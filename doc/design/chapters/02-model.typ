...
两张图为什么必须分开？因为把它们合并就是把#ref(<ch-motivation>)批评过的错误制度化：若拓扑必须追随层次（只许父子相连），则任何跨子树的连接都需要手工逐层转发端口，交叉开关必须与它的每个使用者互为父子——模块层次受连接关系支配，或连接关系受模块层次限制。分开之后，层次按物理与团队边界组织，拓扑按协议逻辑组织，两者互不约束（@req-hierarchy）。
...
连接把一个节点的出边侧与另一个节点的入边侧绑在一起，并携带一个#term[边数意图][cardinality intent]——这条声明代表恰好一条边，还是"由某一侧决定的若干条边"（@req-cardinality，@ch-topology）。协商完成后，每条连接展开为零条或多条#term[边][edge]；边是点对点的最终链路，持有结算出的边参数。
...
#table(
  ...
  [协商],
  [纯函数：输入设计规格，输出协商结果或一个错误（返回类型见@sec-error-semantics）。解析边数、传播参数、结算每条边、计算协议参数、规划端口与连线。],
...
这个切分直接兑现#ref(<req-pure>)与#ref(<req-phases>)。特别地，"在协商完成前读取协商结果"在本模型中*无法表达*：构建期的世界里不存在边参数这种东西；生成器模块想依赖协商结果的部分，被显式建模为协商期计算的协议参数（@sec-two-layer-params），在例化期整体交付。三个阶段之间不存在任何回调回流。
...
协商这一侧与硬件生成那一侧之间，只交换一种数据：每个生成器模块最终交给生成器的#term[完整参数][full parameter]——用户参数与协议参数的合成（@sec-two-layer-params）。zaozi 要求它可序列化；这正对应#ref(<req-serializable>)的目标：每个 IP 拿到一份固定的参数文件，能够脱离整个设计独立例化、独立测试；协商产物可归档、可复现。
...
以上四节给出了骨架：两张图（@req-topology、@req-hierarchy）、三个阶段（@req-pure、@req-phases）、两种模块、一条序列化边界（@req-serializable）。余下各章逐一充实：协议与参数（@ch-protocol）、边数（@ch-topology）、协商算法（@ch-negotiation）、层次穿越与模块生成（@ch-hierarchy）、硬件边界（@ch-hardware）、验证（@ch-verification）以及工具与静态评估（@ch-tooling）；需求与章节的完整对应见@sec-req-map。
