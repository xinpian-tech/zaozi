...
构建期声明节点和记录连接需要框架提供的构建上下文 `DesignBuilder`。设计入口注入该上下文。每个模块以它的用户参数构造：生成器模块在其中声明所用的生成器、inward 与 outward 节点、模块内部参数依赖和验证端点（@sec-generator-module）；结构模块在其中例化子模块并记录 bind。条件拓扑与循环生成的子系统由宿主语言控制流表达。bind 算子 `<-` 只能在该上下文中记录连接。声明的名称默认取自绑定它的 val（sourceinfo，与 zaozi 的实例命名一致）；循环等 val 名不可用的场合以局部 given 覆盖。模块体的返回值是它向外交出端点的唯一通道，类型限定为端点容器：节点、验证端点，及其可选值、序列与字段全为端点容器的积类型——其他值（读取句柄、构建上下文本身）无法离开模块体。设计体返回时，构建器固化为不可变的 `DesignSpec`，构建上下文的生命周期随之结束。
