...
- #term[结构模块][`WrapperModule`]　不带生成器，只用来组织层次：它按用户参数例化子模块，并在其中声明模块之间的连接（@sec-node-conn-proto）。它的电路只有子模块实例、端口和连线；穿过它的连接需要哪些端口和连线，由框架算出并生成（@ch-hierarchy）。
...
实体标识由已命名结构派生：`ModuleId` 是从设计根开始的实例名路径；`ModuleNodeId` 由 `module: ModuleId` 与 `name: NonEmptyString` 组成；`BindId` 由声明顺序和源、目标 `ModuleNodeId` 组成。同一模块内节点名唯一。每个节点唯一关联一条 bind，因此 `ModuleNodeId` 可以确定该节点所在的 `BindId` 和已求解边；一条 bind 同时关联源、目标两个节点。探针源的标识见 @sec-dv-declarations。

每个模块、节点、bind、内部参数依赖和探针源都记录声明处的源码位置，直接采用 sourcecode 库的 `File` 与 `Line` 捕获，不自设位置类型。源码位置只用于诊断，实体身份由稳定标识确定。

== 构建阶段 <sec-build>

构建期声明节点和记录连接需要框架提供的构建上下文 `DesignBuilder`。设计入口注入该上下文。每个模块以它的用户参数构造：生成器模块在其中声明所用的生成器、inward 与 outward 节点、模块内部参数依赖和探针源（@sec-generator-module）；结构模块在其中例化子模块并记录 bind。条件拓扑与循环生成的子系统由宿主语言控制流表达。bind 算子 `<-` 只能在该上下文中记录连接。声明的名称默认取自绑定它的 val（sourceinfo，与 zaozi 的实例命名一致）；循环等 val 名不可用的场合以局部 given 覆盖。模块体的返回值是它向外交出端点的唯一通道，类型限定为端点容器：节点，及其可选值、序列与字段全为端点容器的积类型，以及以字段声明端点的端点类——其他值（读取句柄、构建上下文本身）无法离开模块体。设计体返回时，构建器固化为不可变的 `DesignSpec`，构建上下文的生命周期随之结束。

可复用的模块定义由端点类和定义函数组成：端点类以字段声明节点（字段名即节点名），定义函数绑定注册表条目并转发构建上下文，实例名来自调用点的 val，需要嵌入完整参数的实例名也从该上下文取。携带名称上下文的定义函数体内只含单个模块声明调用；具名声明全部位于端点类中，名称上下文因此不会截获它们。
...
协议的身份就是协议对象本身：bind 两端使用同一个对象由构造保证，无需注册表。设计的生成器注册表由模块树推导——先序首次出现的条目序列，不单独存储。每个条目携带一个名字与 `FullParam` 序列化；名字确定模块命名、去重与链接键，同一名字在一个设计里只对应一个条目。

`DesignSpec` 包含三组内容：

- 固化后的模块树，以及树中各模块的用户参数、不可变 inward、outward 节点规格、模块内部参数依赖、探针源和有向 bind；节点规格保存方向、协议及相应的 `dFn` 或 `uFn`，每条内部依赖保存声明顺序和源码位置；
- 按稳定标识索引的源码位置，以及模块、节点、内部参数依赖、探针源与 bind 的声明顺序。
...
`ResolvedDesign` 保留对应的 `DesignSpec`，并加上协商的全部结果：每条边求出的参数和接口、每个生成器模块的完整参数、跨层端口与连线的计划、FIRRTL 层声明；字段见 @sec-resolved-records 与 @sec-generator-records。例化阶段据此调用生成器；工具导出使用协议对象的序列化（@ch-protocol）。
