...
- #term[连接结构][connection structure]　由#term[总线][bus]、附着到总线上的#term[成员][member]、以及连接它们的桥构成（详见@ch-interconnect）。它表达*可达与协商*：谁与谁经哪条总线相通、地址如何译码、参数在何处协商。成员是设备（桥也是一种设备），各自唯一从属于一个模块。

关键性质是：*连接不受层次约束*。一个位于层次树深处的成员，可以附着到定义在别处的总线上；成员与它所属模块相隔多少层边界，是层次树的事，不是连接结构的事。跨越边界所需的端口，由协商阶段收尾的打洞遍统一规划（@sec-passes，规则详见@ch-hierarchy），不需要设计者逐层转发。
...
两个结构为什么必须分开？因为把它们合并就是把#ref(<ch-motivation>)批评过的错误制度化：若连接必须追随层次（只许父子相连），则任何跨子树的连接都需要手工逐层转发端口，交叉开关必须与它的每个使用者互为父子——模块层次受连接关系支配，或连接关系受模块层次限制。分开之后，层次按物理与团队边界组织，连接按协议逻辑组织，两者互不约束（@req-hierarchy）。
...
#不变量[一次附着的两端必须服从同一协议。跨协议的转换不是附着的属性，而是一个显式的跨协议适配器（@sec-protocol-object）。]
...
为什么禁止"既有子模块又含少量逻辑"的中间形态？因为此类逻辑几乎必然需要引用协商结果（例如按最终成员数量生成仲裁器），而它所处的位置又在协商所需的连接声明之中——两个阶段在同一段代码中相互依赖，阶段边界随之失效。将此类逻辑归入生成器（它以协议参数的形式获得协商结果），纯组合保留给结构模块，三阶段边界才能成立。经验上，所有在结构模块中直接书写逻辑的需求，都可以改写为例化一个小型生成器模块。
...
以上四节给出了骨架：两张图、三个阶段、两种模块、一条序列化边界；它们如何逐条回应#ref(<sec-requirements>)的七条需求，见@sec-req-map。余下各章逐一充实：协议与参数（@ch-protocol）、互连模型（@ch-interconnect）、协商算法（@ch-negotiation）、层次穿越与模块生成（@ch-hierarchy）、硬件边界（@ch-hardware）、验证（@ch-verification）以及工具与静态评估（@ch-tooling）；需求与章节的完整对应见@sec-req-map。
