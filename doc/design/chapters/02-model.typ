...
构建期声明节点和记录连接需要框架提供的构建上下文 `DesignBuilder`。设计入口注入该上下文。每个模块以它的用户参数构造：生成器模块在其中声明所用的生成器、输入与输出节点、模块内部参数依赖和验证端点（@sec-generator-module）；结构模块在其中例化子模块并记录 bind。条件拓扑与循环生成的子系统由宿主语言控制流表达。bind 算子 `<-` 只能在该上下文中记录连接。设计体返回时，构建器固化为不可变的 `DesignSpec`，构建上下文的生命周期随之结束。
...
- 固化后的模块树，以及树中各模块的用户参数、不可变输入、输出节点规格、模块内部参数依赖、验证端点和有向 bind；节点规格保存方向、协议及相应的 `dFn` 或 `uFn`，每条内部依赖保存声明顺序和源码位置；
- 两张注册表的不可变副本；
