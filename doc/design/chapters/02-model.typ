...
第 1 章把参数协商确立为硬件生成流程里一个独立的编译阶段（@sec-explicit-phase）。协商要作为一个能独立运行、能重放的步骤，就得先有"一个设计"的纯数据表示。
...
#图([Triptych 流水线。矩形是阶段，胶囊是阶段间的不可变产物。左右两侧分别是宿主语言闭包与硬件对象的世界，中间是协商。])[
  #syn-diagram(
    spacing: (8mm, 9mm),
...
构建期声明节点、记录连接这些动作，被一个只能由框架入口注入的#term[构建器令牌][builder token]封闭。在宿主语言中，令牌是一个上下文值：
...
禁止"既有子模块又含少量逻辑"的中间形态，是因为此类逻辑几乎必然要引用协商结果（例如按最终 agent 数量生成仲裁器），而它所处的位置又在协商所需的连接声明之中：两个阶段在同一段代码里相互依赖，阶段边界随之失效。含逻辑的部分一律归入生成器（它以协议参数的形式获得协商结果），只含组合的部分留给结构模块，三阶段边界才成立。经验上，所有在结构模块中直接书写逻辑的需求，都可以改写为例化一个小型生成器模块。
