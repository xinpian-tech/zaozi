...
连接把一个节点的出边侧与另一个节点的入边侧绑在一起，并携带一个#term[边数意图][cardinality intent]——这条声明代表恰好一条边，还是"由某一侧决定的若干条边"（@ch-topology）。协商完成后，每条连接展开为零条或多条#term[边][edge]；边是点对点的最终链路，持有结算出的边参数。
