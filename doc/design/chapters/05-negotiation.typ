...
#图([协商的遍。每一遍是纯函数；箭头是唯一的数据流。前五遍确定图的形状，中间四遍确定参数，后三遍规划端口、连线与层（@ch-hierarchy、#ref(<ch-verification>)详述），最终装配为协商结果。])[
  #syn-diagram(
    ...
    node((2, 0), [边数求解 \ #text(size: 8pt, fill: c-dim)[@sec-star-solving]], name: <s>),
...
*校验*在一切计算之前拦截结构错误：连接两端协议一致（类型系统已在构建期拦截绝大多数，此处是防御性复查）；节点角色与其连接侧向相容；以及最重要的——*协商图无环*。参数要沿边单向传播两遍，图必须是有向无环图；环报告与边数依赖环（C8）互相独立，各自呈现完整回路。
...
边数求解后，每个节点两侧的端口向量已经定形。下行传播沿图的拓扑序进行：访问一个节点时，它全部入侧端口的下行参数已经就绪，按角色变换产出它全部出侧端口的下行参数。
...
*边结算*对每条边独立调用其协议的 `negotiate(down, up)`。结算失败即协商停止，错误附带该边两端节点与连接的书写位置。结算成功后，边参数向量按端口索引排列——从此"第几条边"与"哪个参数"永久对应。
...
#term[边视图][`EdgeView`]是该*模块*的只读视图：模块内每个节点 → 它的入侧与出侧边列表，每项含边参数、对端身份与连接位置，按端口索引排序。排序继承自声明序与边数解，因此协议参数的计算和其余一切同样确定。
...
因此单个错误必须自带定位所需的*全部*上下文：类别编号（C1–C9、传播与结算类）、主体（节点/连接/模块的稳定标识）、全部相关书写位置（`SourceLocation`——歧义的双方、环的全程都在其中）、以及当时的计数快照。一个错误的报告是完整的；多个错误的清单不是目标。文本格式规范见@ch-tooling。
