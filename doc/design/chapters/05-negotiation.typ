...
协商是一个纯函数：输入设计规格，输出协商结果或一个错误。它是实现互连模型（@ch-interconnect）的算法：规格给出总线、agent、bind 与落点，协商据此为每个 agent 求解接口、为每条总线算出地址图与可达关系，并装配出交给各 fabric 的清单（@sec-manifest）。贯穿本章的两个词沿前几章的定义：bind 把一个 agent——确切说，它的模块侧*节点*，即 agent 在协议上的接入点——接到总线的一个具名节点上（@sec-attach）；为这次 bind 求解出的那条点对点链路，是协议意义上的一条*边*（@sec-protocol-object）。协商的全部计算发生在这些边上。本章把协商拆成一列顺序执行的遍，规定每一遍的输入、输出与失败模式，然后给出参数传播的精确语义与协议参数的计算规则。

== 遍的流水线 <sec-passes>

#图([协商的遍。每一遍是纯函数；箭头是唯一的数据流。校验拦截结构错误；中间四遍（两遍传播、边求解、协议参数）算出全部参数；清单装配把结果按总线打包；随后三遍规划端口、连线与层（@ch-hierarchy、#ref(<ch-verification>)详述），最终装配为协商结果。])[
  #syn-diagram(
    spacing: (5.5mm, 11mm),
...
参数流天然无环。下行与上行传播（@sec-propagation）各是一次单遍扫描，沿参数流的拓扑序推进，这要求流动图无环——它由结构保证，无需检查。每个 agent 的声明都来自它自身的用户参数，不读任何协商结果；总线只做单向的纯聚合（agent 声明 → 总线 → 回传各 agent），agent 消费回传值，却从不把它喂回自己的声明；桥也一样——它在每条总线上声明的是自己有界的参数（转发窗口、注入的标识空间），不会把甲总线的聚合当作自己在乙总线的声明。于是依赖图恒为有向无环图：自给的声明在源头，聚合在其下游，没有任何参数是自己下游的函数，环无从表达。这也解释了物理上带环的互连为何不成问题：一条边承载完整的双向物理链路（@sec-protocol-interface），环形、网状拓扑的回路属于 fabric 内部，不出现在 agent 结构上；两个模块互为主从时，是两次方向相反的 bind、两条独立的边；物理链路上的回路，落到参数依赖图上仍是无环的。

*清单装配*把求解落成交接物。每条总线收拢自己的结果——每个 agent 的方向、求解接口、地址区域或标识空间、域归属，连同落点：协商不读它、不改它，原样抄入（@sec-placement）——组成 agent 表与可达表，即这条总线的清单（@sec-manifest）。这一遍没有任何求解：每个 agent 在它的具名节点上恰好一个端口，端口的数目、位置与次序在 bind 写下时已经确定，装配只是收拢成表。装配完成随即运行该总线 fabric 的能力校验——校验是清单与策略的纯函数（@sec-manifest），不必等到例化：承载不了的清单在协商期就报出（@sec-error-semantics）。
...
+ *变换是纯函数。*agent 的声明、总线的聚合与分发，服从与求解函数相同的三性质（@sec-protocol-object）：全函数、确定、不读环境。同一规格永远传播出同一组参数。
