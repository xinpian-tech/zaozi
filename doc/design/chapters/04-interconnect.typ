...
Xbar、NoC、直连、时钟树、复位树和电源网格都由生成器模块实现。它们与处理器核、DMA、内存控制器、外设和桥使用相同的模块、节点与 bind：设计源码显式例化模块，模块声明具名端口节点和端口之间的参数依赖，协商结果形成完整参数，zaozi 生成对应硬件。

== 互连生成器的节点 <sec-interconnect-nodes>

一个生成器模块可以声明任意数量的具名输入节点和输出节点；数量在 `DesignSpec` 固化前由普通宿主语言代码和生成器用户参数确定。每个节点表示生成器 IO 中的一个顶层端口，并恰好参与一次设计 bind。

例如，具有两个上游端口和两个下游端口的 `sysNoc` 声明 `cpuIn`、`dmaIn` 两个输入节点以及 `dramOut`、`periphOut` 两个输出节点。`periphXbar` 声明一个输入节点和三个外设输出节点。节点数量已经显式给出，协商无需再从连接算子推导端口数。
...
设计 bind 记录源 `ModuleNodeId`、目标 `ModuleNodeId`、声明顺序和源码位置。源必须是输出节点，目标必须是输入节点，两端使用同一个协议。bind 的方向也是 `Down` 的传播方向，`Up` 沿同一条边反向传播。

每条 bind 直接产生一条边。输出节点恰好作为一次 bind 的源，输入节点恰好作为一次 bind 的目标；未使用的可选端口通过构建期条件代码不声明相应节点。多个同协议端口由多个不同名称的节点表示。

实体标识由已命名结构派生：`ModuleId` 是从设计根开始的实例名路径；`ModuleNodeId` 由 `module: ModuleId` 与 `name: NonEmptyString` 组成；`BindId` 由声明顺序和源、目标 `ModuleNodeId` 组成。同一模块内节点名唯一。每个节点唯一关联一条 bind，因此 `ModuleNodeId` 可以确定该节点所在的 `BindId` 和已求解边；一条 bind 同时关联源、目标两个节点。
...
物理结构中未对外暴露的位置只存在于生成器用户参数和内部实现中。生成器能力校验根据已声明节点及其已求解参数检查端口数、接口能力、拓扑位置和资源容量。
...
参数传播同时使用两类显式关系：从一个输出节点到一个输入节点的 bind，以及模块内部从输入节点到输出节点的参数依赖。`Down` 参数依赖 DAG 按以下方向组成：

- 每条 bind 从源输出节点指向目标输入节点；
- 每条模块内部参数依赖从输入节点指向输出节点。

`Up` 参数依赖 DAG 反转上述全部方向。结构校验检查 `Down` 图无环；`Up` 是它的反向图，自动无环并复用相反的拓扑次序。原始 bind 关系可以因模块具有彼此独立的端口而形成物理上的回路，只要模块内部没有把这些端口连成参数依赖环。

协商器对 `Down` DAG 做稳定拓扑排序，并以逆序求值 `Up` DAG：

- 输出节点的 `dFn` 读取其显式前驱输入节点的 `Down`，产生该输出节点唯一的 `Down`；无前驱时从构建期用户参数产生初始值；
- 输入节点的 `uFn` 读取其显式后继输出节点的 `Up`，产生该输入节点唯一的 `Up`；无后继时从构建期用户参数产生初始值；
- bind 在模块之间传递相应参数；两遍完成后，每条 bind 调用一次 `protocol.negotiate(down, up)` 得到 `Edge`，再调用 `interfaceOf(edge)` 得到 `ProtocolBundle`。

以内存互连为例，`sysNoc` 的每个下游输出节点可以在 `dFn` 中读取所有能够到达该端口的上游输入节点 `Down`，计算事务身份扩展、节点编号或内部表项容量；每个上游输入节点的 `uFn` 可以读取它能够到达的下游输出节点 `Up`，汇聚地址区域、操作能力和位宽约束。不同输出端口的可达集合可以不同，依赖关系与函数均由 `sysNoc` 模块显式声明。
