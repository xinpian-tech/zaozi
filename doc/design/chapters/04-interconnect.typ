...
一条#term[总线][bus]是一个逻辑分组:它命名一组 agent,并承载关于这组 agent 的全部逻辑事实——谁能到达谁、地址如何译码、各 agent 属于哪个时钟域与电源域。参数协商就在总线的 agent 之上运行(@ch-negotiation):agent 的声明在总线上相遇,求解出每个 agent 的接口与整张地址图。总线自身不产生任何硬件——它没有端口、没有连线,是纯粹的规格对象。
...
- *协商。*在每条总线的 agent 上做计算,求解接口与地址图,为每条总线装配一份清单,并就地对清单做 fabric 能力校验(@sec-manifest)——承载不了的清单在此报出。落点不参与计算,原样抄入清单(@sec-placement)。
...
一个直接推论:*fabric 可以各自独立例化。*协商整体先于例化,例化开始时每份清单都已完整;桥两侧的两条总线,各自的 fabric 只取决于各自那份清单,输入互不引用对方的输出,例化因此没有先后依赖——可以分开执行、分开缓存,乃至分别交给不同团队。一个子系统能够脱离整机独立构建、独立测试(@req-ip),机制根源就在这里:它需要的一切都在自己那份清单里。
