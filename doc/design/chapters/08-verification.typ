...
参数只有一股流：探针源（上游端）的下行声明，汇端不回传——被观察者不需要知道谁在观察它。探针连接因此与设计侧的 bind 是*同一个* `<-`：都创建一条边、走同一套下行传播与逐边求解（@ch-negotiation）；差别只在验证协议不设上行，而探针汇（收齐后由 `resolve` 聚合）扮演设计侧总线那样的 nexus。此处需要区分两种"方向"：下行与上行相对*边*而言（@sec-three-param-kinds）；而探针边的*路由*沿层次树向上（@sec-dv-routing）。参数沿边下行、边本身向层次上方走线，两者互相独立、各自成立。
