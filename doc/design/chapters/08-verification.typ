...
验证环境需要观察设计深处的信号：流水线的架构状态供协同仿真比对、总线事务供记分板检查、断言散布在各级。直接从模块内部引出信号破坏封装；在顶层以层次路径字符串引用信号则脆弱且无法参与协商。Syntheke 的方案与设计侧一致：*探针也是图*——用验证协议的节点与连接声明，走同一套打洞机制，并全程处于可整体擦除的层内（R7）。

== 模型 <sec-dv-model>

- #term[探针源][DV source]：位于生成器模块，声明验证协议与上行参数，携带到生成器*探针接口*字段的硬件绑定。生成器的探针接口（@sec-generator-contract）暴露只读引用，不是驱动线。
- #term[探针汇][DV sink]：位于层次树上方的某个模块——通常是顶层附近的验证壳。收齐全部到达的上行参数后，由协议的 `resolve(ups)` 聚合出边参数（@sec-dv-protocol），据此确定汇端聚合接口的形状。

参数只上行。探针没有"下行参数"的对偶概念：被观察者不需要知道谁在观察它。

#图([探针的上行路由。两个深处的探针源（紫）沿层次树向上打洞，汇聚于顶层验证壳的探针汇。沿途每个边界出现带层路径标注的探针端口。])[
  #syn-diagram(
    spacing: (15mm, 7.5mm),
...
#决策([探针连接只允许单连])[
  验证连接只有 `:=` 一个算子，没有星号。设计侧的星号服务于"数量是全局函数"的结构（R3）；而验证观察点应当被*显式枚举*——"自动收集所有可用探针"式的通配会让验证环境的覆盖范围随设计漂移而无声变化，这在验证方法学上是缺陷不是便利。每条探针连接都是一条独立、可审计的显式声明。同构多核之类的批量场景不构成反例：构建期是普通宿主语言代码，对核列表循环生成 $N$ 条单连即可。循环与通配有本质区别——循环枚举的是设计者显式持有的列表，规格中记录的仍是 $N$ 条独立、可审计的连接；通配的匹配范围由框架的图搜索隐式决定，覆盖范围才会随设计漂移。
] <dec-dv-once>

#决策([探针汇必须是探针源的严格祖先])[
  兄弟模块之间的横向探针连接非法。理由有三：其一，上行路由让每条探针的走线路径唯一且无环——沿层次树向上，不需要任何布线决策；其二，一个模块有哪些探针端口只取决于其*内部*内容，与例化它的环境无关——同一个核，在被监听与不被监听的两颗芯片里仍是同一个模块，去重（@sec-dedup）不被观察者破坏；若允许横向连接，核的端口形状将随环境变化，模块复用随之不再成立；其三，探针汇集中于祖先模块，与验证环境的组织方式（顶层协同仿真、子系统级测试台）一致。需要横向观察时，把汇提升到公共祖先，再由汇端分发。
] <dec-dv-ancestor>
...
探针硬件必须能从发布构建中整体消失。Syntheke 采用 FIRRTL 的层机制：每个探针源声明它属于的#term[层路径][layer path]——一条命名链，如 `verification.cosim`、`verification.assert`。层次树上每个模块的*层声明*由框架合并计算：
...
擦除语义：构建时关闭某条层路径，则该层的探针端口、探针连线、以及生成器内绑定到该层的全部验证逻辑一并不发射——设计侧电路逐位不变。这依赖生成器与结构模块两侧对层名的一致（生成器的层接口声明与探针源的层路径由同一参数推导，@sec-generator-contract），一致性在协商期核对。
