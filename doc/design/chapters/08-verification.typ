...
#决策([探针连接只允许单连])[
  验证连接只有 `:=` 一个算子，没有星号。设计侧的星号服务于"数量是全局函数"的结构（@req-cardinality）；而验证观察点应当被*显式枚举*——"自动收集所有可用探针"式的通配会让验证环境的覆盖范围随设计漂移而无声变化，这在验证方法学上是缺陷不是便利。每条探针连接都是一条独立、可审计的显式声明。同构多核之类的批量场景不构成反例：构建期是普通宿主语言代码，对核列表循环生成 $N$ 条单连即可。循环与通配有本质区别——循环枚举的是设计者显式持有的列表，规格中记录的仍是 $N$ 条独立、可审计的连接；通配的匹配范围由框架的图搜索隐式决定，覆盖范围才会随设计漂移。
] <dec-dv-once>
