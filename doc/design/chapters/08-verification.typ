...
验证环境需要观察设计深处的信号：流水线的架构状态供协同仿真比对、总线事务供记分板检查、断言散布在各级。直接从模块内部引出信号破坏封装；在顶层以层次路径字符串引用信号则脆弱且无法参与协商。Syntheke 的方案与设计侧一致：*探针也是图*——用验证协议的节点、同一个 `<-` 声明连接，走同一套打洞与协商机制（@req-verification、@sec-attach），并全程带有 FIRRTL 层标注（层本身的能力见 @sec-layers 的职责划分）。

== 模型 <sec-dv-model>

- #term[探针源][DV source]：位于生成器模块，给出验证协议的下行声明（@sec-dv-protocol）；它就是生成器的一个*探针端口*节点。生成器的探针接口（@sec-generator-contract）暴露只读引用，不是驱动线。
- #term[探针汇][DV sink]：位于层次树上方的某个模块——通常是顶层附近的验证壳。收齐全部到达的声明后，由协议的 `resolve` 聚合出边参数，据此确定汇端聚合接口的形状。

参数只有一股流：探针源（上游端）的下行声明，汇端不回传——被观察者不需要知道谁在观察它。探针连接因此与设计侧的附着是*同一个* `<-`：都创建一条边、走同一套下行传播与逐边结算（@ch-negotiation）；差别只在验证协议不设上行，而探针汇（收齐后由 `resolve` 聚合）扮演设计侧总线那样的 nexus。此处需要区分两种"方向"：下行与上行相对*边*而言（@sec-three-param-kinds）；而探针边的*路由*沿层次树向上（@sec-dv-routing）。参数沿边下行、边本身向层次上方走线，两者互相独立，并不矛盾。
...
#决策([探针连接必须逐条显式枚举])[
  验证观察点必须被*显式枚举*，不提供任何通配收集。设计侧的成员数量随附着增删而自动变化（@sec-attach），服务于集成的敏捷；验证的覆盖范围恰恰相反——"自动收集所有可用探针"式的通配会让验证环境的覆盖范围随设计漂移而无声变化，这在验证方法学上是缺陷不是便利。每条探针连接都是一条独立、可审计的显式声明。同构多核之类的批量场景不构成反例：构建期是普通宿主语言代码，对核列表循环生成 $N$ 条 `<-` 即可。循环与通配有本质区别——循环枚举的是设计者显式持有的列表，规格中记录的仍是 $N$ 条独立、可审计的连接；通配的匹配范围由框架的图搜索隐式决定，覆盖范围才会随设计漂移。
] <dec-dv-once>
