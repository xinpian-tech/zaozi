...
该值随后进入完整参数，由生成器决定硬件形态（例如设备数为零则不生成仲裁树）。与 Diplomacy 的差别只在信息的存放位置：Diplomacy 中它是图上一个无名的中间值，离开整张图无法复现；Syntheke 把它物化进完整参数，换取单个 IP 的独立例化（@req-ip）与按结构键去重（@sec-dedup）。
