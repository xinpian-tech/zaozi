...
设备经中间聚合模块（如中断 Xbar）汇入控制器时，控制器只有一个 inward 节点、一条边，"到达它的中断线总数"不再是拓扑数据，而是整张子图的计算结果。这正是 `Down` 传播的本职：协议把该值建模进 `Down` 类型（@sec-three-param-kinds 的中断行——供出的中断线数量与触发语义）；每级聚合模块的 outward 节点 `dFn` 折叠其可达 inward 节点的 `Down`；到达控制器的那条边求解后，边参数已含全图贡献。控制器的 `computeProtocolParam` 只读本模块 `EdgeView`（@dec-pp-local）即可取得该值——"依赖图的计算结果"与"只读本模块"并不矛盾，因为传播已把图折叠进边。

该值随后经 `combine` 进入完整参数，由生成器决定硬件形态（例如设备数为零则不生成仲裁树）。与 Diplomacy 的差别只在信息的存放位置：Diplomacy 中它是图上一个无名的中间值，离开整张图无法复现；Syntheke 把它物化进完整参数，换取单个 IP 的独立例化（@req-ip）与按结构键去重（@sec-dedup）。
