...
`Down/Up/Edge` 定义为协议对象的*类型成员*：边上的全部类型信息由一个协议值一次性给出，节点、连接、算法的签名只需携带 `P <: Protocol` 一个类型参数，把 `Down/Up/Edge` 及两侧变体都收在这个协议值里。协议库作者定义一个对象；使用者引用一个名字。类型之外，协议定义必须以文字写明它的两端指认（@sec-three-param-kinds 表的格式）：谁是上游、两端各是什么角色、`Down` 与 `Up` 各承载哪端的哪些供给与需求。缺少这段指认，使用者只能靠猜测确定连接方向。

`negotiate` 承担的职责对应三条必须遵守的性质：

+ *全函数。*对每一种参数组合都有定义：不兼容的组合（下游不支持上游要发的操作、地址集越界……）结算为描述性的参数冲突，协商器将其转为带两端书写位置的报错。
...
协议还要给出：给定一条已结算的边，这条链路对应的硬件形状。它用一个纯数据的描述表达——#term[协议接口][protocol interface]：
