...
`Down/Up/Edge` 定义为协议对象的*类型 agent*：边上的全部类型信息由一个协议值一次性给出，节点、连接、算法的签名只需携带 `P <: Protocol` 一个类型参数，把 `Down/Up/Edge` 及两侧变体都收在这个协议值里。协议库作者定义一个对象；使用者引用一个名字。类型之外，协议定义必须以文字写明它的两端指认（@sec-three-param-kinds 表的格式）：谁是上游、两端各是什么角色、`Down` 与 `Up` 各承载哪端的哪些供给与需求。缺少这段指认，使用者只能靠猜测确定连接方向。

`negotiate` 承担的职责对应三条必须遵守的性质：

+ *全函数。*对每一种参数组合都有定义：不兼容的组合（下游不支持上游要发的操作、地址集越界……）求解为描述性的参数冲突，协商器将其转为带两端书写位置的报错。
+ *确定性。*只读两个入参，不读任何环境——没有全局配置、没有时间戳、没有随机性。同一规格永远求解出同一设计。
...
`resolve` 在汇端一次性看到全部到达的声明并聚合——与设计协议逐边求解不同，因为探针汇的职责本来就是收集全体。`DVProtocol` 与 `Protocol` 是并列的两个契约，*没有*子类型关系：强行共用一套类型，只会使 `Up` 与 `negotiate` 两个 agent 失去意义。一个协议库要同时服务设计连接与验证探针时，分别给出两个对象即可（内部尽可复用共同的参数定义）。验证协议的连接规则、层机制与打洞细节见@ch-verification。
