...
+ 模块的硬件接口、探针接口、层结构都*只看参数就能算出*——协商期要在不生成任何电路的前提下规划端口（@sec-punch-planning）、核对端口结构（@sec-generator-module）；如果接口形状要等电路造完才知道，这两件事都无从谈起。
...
abstract class GeneratorModule[UP, PP, FP, I](
  ...
  // 声明具名节点（名 + 协议 + 角色）；节点即生成器的一个端口，无需另行绑定接口字段。
```

*节点即端口，不再有单独的"硬件绑定"这一层。*旧式设计把节点（协商图上的接入点）与生成器的 IO 字段当作两样东西，要一个映射函数把前者对到后者的哪个字段；这个映射之所以非平凡，一是因为一个节点可能展开成多个端口（基数），二是因为一个接口可能有多个同协议字段。基数已经取消——一个节点恰好一个端口；而"多个同协议端口"由不同的节点*名字*区分。于是映射退化为按名对应：生成器的每个具名节点，例化后就是它 IO 上同名的那个端口，节点结算出的接口（由 `interfaceOf` 从边参数导出，@sec-protocol-interface）就是该端口的线形状。例化期框架据此登记每条边的硬件端点，供连线计划引用。

#决策([端口结构校验在例化期进行])[
  生成器为每个节点产出的端口，其硬件类型必须与该节点边参数的 `interfaceOf(edge)` 结构一致（字段名、宽度、翻转逐层相同）。校验时机选在例化期、方式选结构比对，而非把协议边参数与硬件类型在类型系统层面绑定：后者使每个协议库与每个生成器库强耦合，任何一侧演化都牵动另一侧的类型签名。失配报错携带节点身份、连接位置、期望与实际形状的差异路径。
] <dec-binding-check>
...
#table(
  ...
  [*从不序列化*], [参数变换函数、合成与推导函数——一切闭包。], [闭包活在单次进程内。硬要写进文件，只能把代码存成文本、加载时再求值——这构成宏系统而非参数，复现性与可审计性同时丧失。],
)
