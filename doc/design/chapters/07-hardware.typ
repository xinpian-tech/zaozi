...
#决策([端口结构校验在例化期进行])[
  生成器为每个节点产出的端口，其硬件类型必须与该节点边参数的 `interfaceOf(edge)` 结构一致（字段名、宽度、翻转逐层相同）。校验时机选在例化期、方式选结构比对，而非把协议边参数与硬件类型在类型系统层面耦合：这会使每个协议库与每个生成器库强耦合，任何一侧演化都牵动另一侧的类型签名。失配报错携带节点身份、连接位置、期望与实际形状的差异路径。
] <dec-binding-check>
