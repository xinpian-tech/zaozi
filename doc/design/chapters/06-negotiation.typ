...
以内存互连为例，Xbar 或 NoC 为每个下游 outward 节点声明它可由哪些上游 inward 节点到达。该 outward 节点的 `dFn` 可以聚合这些 inward 节点的事务身份需求，计算 ID 扩展、节点编号或内部表项容量；每个上游 inward 节点的 `uFn` 则聚合其可达 outward 节点的地址区域、操作能力和位宽约束。地址重叠、不可达或身份空间无法分配等冲突由正在执行的端口参数函数报告 N2。
...
#table(
  ...
  [*N2* 参数传播失败], [`Down` 或 `Up` 传播], [`dFn` 或 `uFn` 返回约束冲突，例如地址区域重叠、请求地址不可达或事务身份空间无法分配。], [模块、outward 或 inward 节点、传播方向、相关依赖与 bind、有序输入快照、冲突描述和源码位置。],
  [*N3* 边或验证求解失败], [边与验证求解], [设计边的 `negotiate` 或探针汇的 `resolve` 返回参数冲突。], [设计边：`BindId`、`Down`、`Up` 与源码位置；探针汇：`DVSinkId`、有序 `Down` 与相关源码位置；协议给出的冲突描述。],
  [*N4* 节点或 bind 非法], [校验], [节点引用不存在；outward 节点未恰好作为一次 bind 的源；inward 节点未恰好作为一次 bind 的目标；bind 两端方向不符；或声明 bind 的结构模块不是两端节点所在模块的祖先。], [相关 `ModuleNodeId`、`BindId`、声明 bind 的模块、实际 bind 次数和源码位置。],
