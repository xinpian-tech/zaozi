...
每条设计 bind 的源、目标节点必须存在，声明该 bind 的结构模块必须是两端节点所在模块的祖先（@sec-node-conn-proto）。源节点方向为输出，目标节点方向为输入，两端协议匹配；每个输出节点恰好作为一次 bind 的源，每个输入节点恰好作为一次 bind 的目标。节点的数量在构建期已经固定，结构校验分别核对每个输出节点在 bind 源中出现一次、每个输入节点在 bind 目标中出现一次。

模块内部只保存一份从本模块输入节点指向输出节点的参数依赖边集；每条依赖带声明顺序和源码位置。输出节点的前驱与输入节点的后继都从该边集派生，按节点声明顺序排列。重复依赖边非法。每个输出节点必须携带 `dFn`，每个输入节点必须携带 `uFn`，函数字段不可选；函数可读的节点集合与依赖边集来自同一次声明（@sec-generator-module）。

`Down` 参数依赖 DAG 由以下两类边组成：每条 bind 从源输出节点指向目标输入节点；每条模块内部参数依赖从输入节点指向输出节点。`Up` 参数依赖 DAG 反转上述全部方向。结构校验检查 `Down` 图无环；`Up` 是它的反向图，自动无环。稳定拓扑排序在多个节点均可选择时，采用模块的层次树先序和节点声明顺序打破平局；`Up` 直接使用同一拓扑序的逆序。检测到环时，错误包含环上的全部 `ModuleNodeId`、`BindId`、模块内部参数依赖和源码位置。
...
协商返回 `Either[NonEmptyVector[NegotiationError], ResolvedDesign]`。传播只执行全部依赖值都已就绪的节点；同一就绪前沿及其它互不依赖分支中的失败可以一并收集，受失败值阻断的后继不执行，也不产生衍生错误。`Down` 与 `Up` 两遍互不依赖，各自继续执行仍可求值的分支。当前阶段可确定的全部失败按 @ch-tooling 的规范化顺序排列后返回，后续阶段停止执行。
...
#table(
  ...
  [*N4* 节点或 bind 非法], [校验], [节点引用不存在；输出节点未恰好作为一次 bind 的源；输入节点未恰好作为一次 bind 的目标；bind 两端方向不符；或声明 bind 的结构模块不是两端节点所在模块的祖先。], [相关 `ModuleNodeId`、`BindId`、声明 bind 的模块、实际 bind 次数和源码位置。],
