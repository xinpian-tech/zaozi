...
协商成功返回 `ResolvedDesign`；发现首个错误时立即以携带该 `NegotiationError` 的异常终止，不收集后续错误。协议对象与端口参数函数仍以值（`Left`）表达参数冲突——那是协议作者的表达通道；协商器收到即抛。用户代码抛出的异常不属于 N1–N10：它是该代码自身的缺陷，原样穿透并保留其调用栈。
