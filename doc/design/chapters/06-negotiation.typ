...
凡是声明处即可判定的契约在构建阶段当场检查、当场抛出，不进入协商：同一作用域内声明名重复；名称形状非法（@sec-port-naming）；在生成器 body 内声明子模块或 bind（生成器模块是叶子，@sec-module-kinds）；模块内部参数依赖的节点不在本模块、方向非法或依赖重复；跨协议引用的目标不在本模块，或同一节点上引用名重复；一个节点声明多个参数函数。协商阶段只报告需要全局视角才能判定的问题：
