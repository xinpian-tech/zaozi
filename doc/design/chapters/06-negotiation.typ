...
+ 核对生成器名字唯一性，校验名称、节点方向、bind、协议匹配和模块内部参数依赖；
...
结构校验先核对生成器注册表——由模块树推导的先序条目序列。同一模块内的子实例名唯一，节点名与探针源名唯一；每个生成器名字对应一个注册表条目。
...
凡是声明处即可判定的契约在构建阶段当场检查、当场抛出，不进入协商：同一作用域内实例名或端点名重复；名称形状非法（@sec-port-naming）；在生成器 body 内声明子模块或 bind（生成器模块是叶子，@sec-module-kinds）；模块内部参数依赖的端点不在本模块、方向非法或依赖重复；跨协议引用的目标不在本模块，或同一节点上引用名重复；一个节点声明多个参数函数。协商阶段只报告需要全局视角才能判定的问题：
