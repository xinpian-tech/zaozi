...
协商成功返回 `ResolvedDesign`；发现首个错误时立即以异常终止，不收集后续错误。异常消息直接陈述问题本身，并内联主体的稳定标识、全部相关源码位置（`SourceLocation`）及参数快照；没有错误类别或编号。协议对象与端口参数函数仍以值（`Left`）表达参数冲突——那是协议作者的表达通道；协商器收到即抛。用户代码抛出的异常不由协商器包装：它是该代码自身的缺陷，原样穿透并保留其调用栈。

凡是声明处即可判定的契约在构建阶段当场检查、当场抛出，不进入协商：同一作用域内实例名或端点名重复；模块内部参数依赖的端点不在本模块、方向非法或依赖重复；跨协议引用的目标不在本模块；一个节点声明多个参数函数。协商阶段只报告需要全局视角才能判定的问题：
