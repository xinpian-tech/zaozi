...
协商的全部中间态与结果都是普通数据（@sec-passes），这不该只是实现细节，而应兑现为工具能力：可导出、可可视化、可静态评估、错误信息可读。本章规定产物格式的骨架与确定性承诺。格式细节允许随实现演进；*确定性承诺不允许*。

== 导出 <sec-export>

协商结果可按需导出四份 JSON，各自独立成立：

- *拓扑*（`topology.json`）：模块树、总线、成员、附着。每个实体带稳定标识与书写位置。
- *边*（`edges.json`）：每条边的两端（成员标识与它所落的具名节点）、协议名、协议自述的渲染元数据（@sec-protocol-object 的 `render`）。
...
从同一数据可导出两种图（GraphML / DOT）：*协商前视图*呈现意图——总线、成员与附着；*协商后视图*呈现事实——结算出的边、地址图与结算参数的摘要标注。层次树映射为嵌套子图，边的颜色与标签取自协议的渲染元数据。#ref(<ch-interconnect>)的两级设计例题里那些图，工具都应能从对应规格自动重现。

== 静态评估 <sec-static-eval>

协商结果连同拓扑构成一个可直接分析的性能模型（@req-perf）。以带宽评估为例：一段链路的供给能力是结算位宽与所在时钟域频率的乘积；它承载的需求是全部流经它的发起者所声明流量之和（哪些发起者流经哪一段，由地址集合与拓扑决定）；逐段比对两者，即得瓶颈与富余。评估所需的每一项事实——位宽、频率、流量声明——都是协商参数，随协商结果一并导出（@sec-export）。
...
```text
error[地址重叠] 两个响应成员的地址区域相交
  总线 soc.mbus 上：
    Soc.scala:41   dram   服务 0x8000_0000 – 0xFFFF_FFFF
    Soc.scala:47   sram   服务 0x9000_0000 – 0x9000_FFFF
  相交区间 0x9000_0000 – 0x9000_FFFF 无法唯一译码。收窄任一成员的地址区域，使二者不相交。
```

版式的三条强制规则：*全部*位置都列出（歧义的双方、环的全程），用户无需自行查找其余位置；涉及数量或容量的错误（如能力校验）附当前快照；修复建议限一句话，不展开成教程。
