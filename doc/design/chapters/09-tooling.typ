...
从同一数据可导出两种图（GraphML / DOT）：*协商前视图*呈现意图——节点、连接与边数算子；*协商后视图*呈现事实——展开的边、结算参数的摘要标注。层次树映射为嵌套子图，边的颜色与标签取自协议的渲染元数据。#ref(<ch-topology>)例题里的每一张图，工具都应能从对应规格自动重现。

== 静态评估 <sec-static-eval>

协商结果连同拓扑构成一个可直接分析的性能模型（@req-static-eval）。以带宽评估为例：一段链路的供给能力是结算位宽与所在时钟域频率的乘积；它承载的需求是全部流经它的发起者所声明流量之和（哪些发起者流经哪一段，由地址集合与拓扑决定）；逐段比对两者，即得瓶颈与富余。评估所需的每一项事实——位宽、频率、流量声明——都是协商参数，随协商结果一并导出（@sec-export）。
...
错误报告（@sec-error-semantics）的文本渲染遵守统一版式：首行是类别编号与一句话结论；中间列出该错误的全部相关书写位置，每行一个事实；末行给出可行动的修复方向。示例（C3，参数表长度不匹配）：
