...
#table(
  ...
  [探针是一等协议：协商期规划路由与层，逐叶上提为顶层探针端口（@ch-verification、@sec-dv-routing）。],
  [模块身份与去重],
  [例化后由编译器按结构去重，辅以 `CloneLazyModule` 等手工手段。],
  [生成器定义按链接键（生成器名字，规范化完整参数）共享；结构模块每实例一份，网表级合并交给 FIRRTL 编译流程（@sec-dedup）。],
  [产物边界],
...
#决策([不提供自动监视器插入])[
  Syntheke 不提供 `NodeImp.monitor` 的对应机制。监视器拆解为三件已有之物：观测由探针源承担（协议库可为端口提供镜像探针的标准声明）；检查逻辑在设计之外，经顶层探针端口接入；可移除性由 FIRRTL 层承担。自动插入本身属于 @sec-diplomacy 所删除的隐式行为。
] <dec-no-auto-monitor>
