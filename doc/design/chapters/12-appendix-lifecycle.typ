...
#决策([不提供自动监视器插入])[
  Syntheke 不提供 `NodeImp.monitor` 的对应机制。监视器拆解为三件已有之物：观测由探针源承担（协议库可为端口提供镜像探针的标准声明）；检查硬件是普通的验证生成器模块，具有完整参数、可独立例化测试并按链接键共享定义；可移除性由 FIRRTL 层承担。自动插入本身属于 @sec-diplomacy 所删除的隐式行为。
] <dec-no-auto-monitor>
