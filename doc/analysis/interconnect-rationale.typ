...
- *attach*:唯一的连接原语 `<-`,把一个设备的节点绑到一条总线的落点上。方向不写进算子——设备的节点自己声明是发起者还是响应者(协议的两端指认),`<-` 只说"谁承载谁"。数量只是附着的条数:*没有基数算子、没有 resolveStar、没有待解的端口数*(#ref(<sec-fail-role>) 保留下来的方向信息由节点角色承载,不再需要每条边上人工挑算子)。更进一步:连接与硬件是*同一个节点*的两面——绑上一个节点,就接上了它的 IO,不必再有一层"节点 → 接口字段"的硬件绑定。旧模型里那层映射之所以非平凡、要单独存在,恰恰也是因为基数(一个节点可展开成多个端口);基数一去,它与四算子一同退场。

#图([设备经唯一的 attach 原语 `<-` 绑到总线,总线即显式的 nexus。方向由节点角色给出(发起者、响应者),算子不带方向;多重性 = 附着条数(此处发起侧 2、响应侧 2)。总线由下游的 fabric 兑现——fabric 消费清单,不在成员之列。])[
  #syn-diagram(
    spacing: (16mm, 6mm),
    node((0, 0), [CPU], name: <a>),
    node((0, 1), [DMA], name: <b>),
