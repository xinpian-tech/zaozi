...
#table(
  ...
  [谁与谁连接], [设计源码中的显式 bind],
  [端点参数如何变换或聚合], [生成器模块声明 inward 到 outward 的参数依赖；outward、inward 节点分别定义 `dFn`、`uFn`],
...
Xbar、NoC、直连、时钟树、复位树和电源网格都由设计源码显式例化。对应的生成器模块在层次树中声明具名 inward、outward 节点及模块内部参数依赖；这些节点参与 bind，模块接收完整参数并生成硬件。

#图([Xbar、NoC 与其它硬件一样由生成器模块实现。实线 bind 分别连接具名端口节点；模块框内的点线是 inward 到 outward 的参数依赖。])[
  #syn-diagram(
    spacing: (11mm, 7mm),
...
Xbar 也按同一规则处理；外部端口数量表现为相应数量的具名 inward、outward 节点。

= 桥与边界 <sec-bridge>

两段互连之间通过桥连接。桥由生成器模块实现，在两侧分别声明 inward 节点与 outward 节点，并声明二者之间的参数依赖。地址重映射、位宽转换、时钟域转换、电源隔离和协议转换都属于桥的内部逻辑。

生成器模块（包括桥）构成后端切分边界。每个可交付 IP 或子系统拿到自己的完整参数和端口契约；跨边界连线由协商后的端口计划生成。

= 结论 <sec-conclusion>

连接结构由显式例化的模块、模块声明的具名 inward 与 outward 节点、从一个 outward 节点到一个 inward 节点的 bind，以及模块内部从 inward 到 outward 的参数依赖组成。

bind 与模块内部参数依赖组成 `Down` DAG，反向组成 `Up` DAG；outward 节点的 `dFn` 与 inward 节点的 `uFn` 在两个方向上变换或聚合参数，两遍完成后每条 bind 独立调用协议的 `negotiate`。Xbar、NoC、直连、时钟树和电源网格都使用同一套模块、节点与参数传播规则。
